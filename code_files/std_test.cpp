...
#include "../code_files/include/snprintf.h"
...
Test **create_test(Dtest *dtest, unsigned int tsamples,unsigned int psamples)
{
 ...
 for(i=0;i<dtest->nkps;i++)
 {
   ...
   if(all == YES || psamples == 0)
   {
	 ...
	 if (newtest[i]->psamples < 1) 
		 newtest[i]->psamples = 1;
   } 
   else 
   {
...
void add_2_test(Dtest *dtest, Test **test, int count)
{
 ...
 {
    dtest->test(test, i);
	progressTest +=  progressGeneralInc / imax;
 }
...
void std_test(Dtest *dtest, Test **test)
{
  ...
  */
 for(j = 0;j < dtest->nkps;j++)
   if(test[j]->ks_pvalue > pmax) 
	   pmax = test[j]->ks_pvalue;
