...
double distance(const dTuple a, const dTuple b, unsigned int dim)
{
  int i;
...
int rgb_minimum_distance(Test **test, int irun)
{
  ...
  */
 if((rgb_md_dim % 2) == 0){
   dvolume = pow(PI,(double)rgb_md_dim/2)*pow(mindist,(double)rgb_md_dim)/gsl_sf_fact(rgb_md_dim/2);
 } else {
   dvolume = 2.0*pow(2.0*PI,(double)(rgb_md_dim-1)/2)*pow(mindist,(double)rgb_md_dim)/gsl_sf_doublefact(rgb_md_dim);
 }
