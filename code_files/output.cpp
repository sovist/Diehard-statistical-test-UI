 ...
 */
 ...
 if(strncmp("file_input",gsl_rng_name(rng),10) == 0)
 {
   ...
   {
     fileRewound = file_input_get_rewind_cnt(rng);
 ...
 for(i=0;i<dtest->nkps;i++){
   ...
   if(tflag & TASSESSMENT){
     ...
     if(test[i]->ks_pvalue < Xfail || test[i]->ks_pvalue > 1.0 - Xfail)
	 {
	   ...
	   testRez = Diehard::TestRezult::Failed;
     } 
	 else if(test[i]->ks_pvalue < Xweak || test[i]->ks_pvalue > 1.0 - Xweak)
	 {
	   ...
	   testRez = Diehard::TestRezult::Weak;
     } 
	 else 
	 {
       if(tflag & TNO_WHITE)
	   ...
	   testRez = Diehard::TestRezult::Passed;
     }
     field++;
	 Globals::_tableForm->AddLineInTable(dtest, test[i], testRez);
   }
