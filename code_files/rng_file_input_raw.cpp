...
unsigned int first_file_input_raw_set = 1;
...
static unsigned long int file_input_raw_get(void *vstate)
{
 ...
 if(state->fp != NULL) {
   ...
   if( fread(&iret, sizeof(unsigned int), 1, state->fp) != 1)
   {
   ...
   state->rtot++;
   if(verbose){
     fprintf(stdout,"# file_input() %u: %u/%u -> %u\n",(unsigned int)state->rtot,(unsigned int)state->rptr,(unsigned int)state->flen,(unsigned int)iret);
   }
...
static void file_input_raw_set (void *vstate, unsigned long int s)
{
 ...
 if( first_file_input_raw_set)
 {
   ...
   if(S_ISREG(sbuf.st_mode))
   {
      ...
      */
     state->flen = System::IO::FileInfo(gcnew System::String(filename)).Length/sizeof(unsigned int);/*sbuf.st_size/sizeof(unsigned int);*/
     filecount = state->flen;
     if (filecount < 16) 
	 {
		 throw gcnew System::Exception(System::String::Format("file {0} is too small", gcnew System::String(filename)));
 ...
 if (state->fp == NULL)
 {
    ...
    */
   if ((inputFile = state->fp = fopen(filename,"rb")) == NULL) 
   {
	   throw gcnew System::Exception(System::String::Format("Cannot open {0}.", gcnew System::String(filename)));
 ...
 else 
 {
    ...
    */
   if(state->flen >= 0 && state->rptr >= state->flen)
   {
     rewind(state->fp);
     state->rptr = 0;
     state->rewind_cnt++;
