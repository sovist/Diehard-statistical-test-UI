...
#include "../code_files/include/getopt.h"
...
int
_getopt_internal (int argc, char *const *argv, const char *__optstring, const struct option *__longopts, int *__longind, int __long_only)
     /*int argc;
     char *const *argv;
     const char *optstring;
  ...
  if (optind == 0 || !__getopt_initialized)
    {
      ...
      __optstring = _getopt_initialize (argc, argv, __optstring);
...
# define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0')
  ...
  if (__longopts != NULL
      && (argv[optind][1] == '-'
          || (__long_only && (argv[optind][2] || !my_index (__optstring, argv[optind][1])))))
    {
      char *nameend;
      ...
      for (p = __longopts, option_index = 0; p->name; p++, option_index++)
...
%s: option `%c%s' doesn't allow an argument\n"),
      ...
      if (!__long_only || argv[optind][1] == '-'
          || my_index (__optstring, *nextchar) == NULL)
        {
          if (print_errors)
            {
#if defined _LIBC && defined USE_IN_LIBIO
  ...
  {
    char c = *nextchar++;
    char *temp = my_index (__optstring, c);
