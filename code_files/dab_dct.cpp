...
#include <gsl/gsl_fft_real.h> // for int gsl_fft_real_radix2_transform (double data[], size_t stride, size_t n);
...
int dab_dct(Test **test,int irun)
{
 ...
 for (j=0; j<test[0]->tsamples; j++) {
   ...
   dct[0] -= mean;
   dct[0] /= sqrt(2.0);  // Experimental + guess; seems to be correct.
...
void fDCT2(const unsigned int input[], double output[], size_t len) {
 ...
 for (i = 0; i < len; i++) {
   for (j = 0; j < len; j++) {
     output[i] += (double) input[j] * cos((M_PI /*3.14159265358979323846264338328*/ / len) * (0.5 + j) * i);
   }
 }
}
...
void iDCT2(const double input[], double output[], size_t len) {
 ...
 for (i = 0; i < len; i++) {
   double sum = 0;
   for (j = 0; j < len; j++) {
     sum += input[j] * cos(((M_PI /*3.14159265358979323846264338328*/ * j) / len) * (0.5 + i));
   }
