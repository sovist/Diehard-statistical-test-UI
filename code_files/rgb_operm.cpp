...
unsigned int piperm(size_t *data,int len);
void make_cexact();
void make_cexpt();
int nperms,noperms;
...
int rgb_operm(Test **test,int irun)
{
 ...
 int i,j,n,nb,iv,s;
 unsigned int csamples;   /* rgb_operm_k^2 is vector size of cov matrix */
 unsigned int *count,ctotal; /* counters */
 unsigned int size;
 ...
 count = (unsigned int *)malloc(csamples*sizeof(unsigned int));
...
unsigned int piperm(size_t *data,int len)
{
 ...
 unsigned int i,j,k,max,min;
 unsigned int pindex,uret,tmp;
 ...
 for(i=0;i<nperms;i++){
   if(memcmp(data,lookup[i]->data,len*sizeof(unsigned int))==0){
     /* Not cruft, but off:
     MYDEBUG(D_RGB_OPERM){
       printf("# piperm(): ");
       gsl_permutation_fprintf(stdout,lookup[i]," %u");
