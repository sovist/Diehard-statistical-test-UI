...
void
file_input_set_rtot(gsl_rng *rng, unsigned int value)
{
...
static unsigned long int file_input_get (void *vstate)
{
 ...
 if(state->fp != NULL) {
   ...
   state->rtot++;
   if(verbose){
     fprintf(stdout,"# file_input() %lu: %lu/%lu -> %u\n", state->rtot, state->rptr,state->flen,(unsigned int)iret);
   }
...
static void file_input_set (void *vstate, unsigned long int s)
{
 ...
 else 
 {
    ...
    */
   if(state->rptr >= state->flen && state->rptr >= 0 && state->flen >= 0)
   {
     rewind(state->fp);
     state->rptr = 0;
     state->rewind_cnt++;
