...
void run_all_tests()
{
  ...
  */
 int ntmin, ntmax, ntsave, saveProgressGeneralInc;
 ...
 dh_test_types[dtest_num]->nkps;

 for(dtest_num=0; dtest_num < dh_num_diehard_tests; dtest_num++)
   if(dh_test_types[dtest_num] && SelTest[dtest_num])
   {
	 progressTest = 0;
     execute_test(dtest_num);
	 progressTest = 0;
	 progressGeneral += progressGeneralInc; 
   }
     ...
     case 200:
	    ...
	    * not...
		 ...
		 else 
		 {
		   ...
		   saveProgressGeneralInc = progressGeneralInc;
		   progressGeneralInc = System::Math::Floor(progressGeneralInc / ((ntmax - ntmin) + 1));
           for(ntuple = ntmin; ntuple <= ntmax && SelTest[dtest_num]; ntuple++)
             execute_test(dtest_num);
           /*
            * This RESTORES ntuple = 0, which is the only way we could have
            * gotten here in the first place!
            */
		   progressGeneralInc = saveProgressGeneralInc;
           ntuple = 0;
	     }
       }
       break;
