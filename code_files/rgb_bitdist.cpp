...
int rgb_bitdist(Test **test,int irun)
{
 ...
 unsigned int nb;          /* number of bits in a tested ntuple */
 unsigned int value_max;   /* 2^{nb}, basically (max size of nb bit word + 1) */
 unsigned int bsamples;    /* The number of non-overlapping samples in buffer */
 unsigned int value;       /* value of sampled ntuple (as a uint) */
 unsigned int mask;

 /* Look for cruft below */

 unsigned int b,t,i;   /* loop indices? */
 unsigned int ri;
  ...
  */
 value_max = (unsigned int) pow(2.0,(double)nb);
 MYDEBUG(D_RGB_BITDIST){
 ...
 count = (unsigned int *)malloc(value_max*sizeof(unsigned int));
  ...
  * bitcount.
  */
 memset(count,0,value_max*sizeof(unsigned int));
 for(t=0;t<tsamples;t++){
   ...
   for(i=0;i<value_max;i++){
      ...
      if (count_i)
	{
      ...
      MYDEBUG(D_RGB_BITDIST){
	 printf("# rgb_bitdist(): vtest[%u].x[%u] = %u\n",i,count[i],(unsigned int)vtest[i].x[count[i]]);
     }
   }
   MYDEBUG(D_RGB_BITDIST){
