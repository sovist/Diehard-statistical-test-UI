...
int diehard_bitstream(Test **test, int irun)
{
 ...
 unsigned int i,j,t,boffset,coffset;
 Xtest ptest;
 char *w;
 unsigned int *bitstream,w20,wscratch,newbyte;
 unsigned char *cbitstream = 0;
 unsigned int overlap = 1;  /* Leftovers/Cruft */
...
#define BS_NO_OVERLAP 1310722
 ...
 if(overlap){
   ptest.sigma = 428.0;
   bitstream = (unsigned int *)malloc(BS_OVERLAP*sizeof(unsigned int));
   ...
   ptest.sigma = 290.0;
   bitstream = (unsigned int *)malloc(BS_NO_OVERLAP*sizeof(unsigned int));
   ...
   if(overlap){
     ...
     */
	 ...
	 i++;
       ...
       wscratch = (unsigned int) (cbitstream[i] >> 4);    /* Get first 4 bits of next byte */
       MYDEBUG(D_DIEHARD_BITSTREAM) {
         printf("# diehard_bitstream: wscratch = ");
         dumpuintbits(&wscratch, 1);
         printf("\n");
       ...
       wscratch = (unsigned int) cbitstream[i];
       MYDEBUG(D_DIEHARD_BITSTREAM) {
