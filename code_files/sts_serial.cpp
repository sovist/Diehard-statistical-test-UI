...
#include "../code_files/include/static_get_bits.h"
...
int sts_serial(Test **test,int irun)
{
 ...
 unsigned int bsize;       /* number of bits/samples in uintbuf */
 unsigned int nb,nb1;          /* number of bits in a tested ntuple */
 unsigned int value;       /* value of sampled ntuple (as a uint) */
 unsigned int mask;        /* mask in only nb bits */
 unsigned int bi;          /* bit offset relative to window */

 /* Look for cruft below */

 unsigned int i,j,m,t;            /* generic loop indices */
 unsigned int ctotal;  /* count of any ntuple per bitstring */
 double **freq,*psi2,*delpsi2,*del2psi2;
 double pvalue;
 unsigned int *uintbuf;
 unsigned int window;  /* uint window into uintbuf, slide along a byte at at time. */
 ...
 for(m = 1;m < nb1;m++) {
   freq[m] = (double *)malloc(pow(2,(double)m)*sizeof(double));
   memset(freq[m],0,pow(2,(double)m)*sizeof(double));
 }
  ...
  */
 uintbuf = (unsigned int *)malloc((tsamples+1)*sizeof(unsigned int));
  ...
  */
 bsize = tsamples*sizeof(unsigned int)*CHAR_BIT;
 MYDEBUG(D_STS_SERIAL){
 ...
 MYDEBUG(D_STS_SERIAL){
   printf("# sts_serial(): %u:  ",(unsigned int)tsamples);
 ...
 for(m=1;m<nb1;m++){
   ...
   psi2[m] = pow(2,(double)m)*psi2[m]/bsize - bsize;
   MYDEBUG(D_STS_SERIAL){
 ...
 for(m=2;m<nb1;m++){
   ...
   pvalue = gsl_sf_gamma_inc_Q(pow(2,(double)m-2),delpsi2[m]/2.0);
   if(irun == 0){
     test[j]->ntuple = m;
   }
   ...
   if(m>2){
     pvalue = gsl_sf_gamma_inc_Q(pow(2,(double)m-3),del2psi2[m]/2.0);
     if(irun == 0){
       test[j]->ntuple = m;
     }
     test[j++]->pvalues[irun] = pvalue;
