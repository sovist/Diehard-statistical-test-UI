...
unsigned int roll(){
  unsigned int d = 1 + gsl_rng_uniform_int(rng,6);
  return d;
}
...
int diehard_craps(Test **test, int irun)
{
 ...
 unsigned int i;
 unsigned int point,throw_,tries,wins;
 double sum,p;
 Xtest ptest;
 Vtest vtest;
 ...
 vtest.y[0] = sum;
 for(i=1;i<20;i++){
   vtest.y[i] = (27.0*pow(27.0/36.0,(double)i-1) + 40*pow(13.0/18.0,(double)i-1) +
                55.0*pow(25.0/36.0,(double)i-1))/648.0;
   sum += vtest.y[i];
 }
 vtest.y[20] = 1.0 - sum;
 /*
