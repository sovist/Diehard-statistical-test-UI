...
int diehard_oqso(Test **test, int irun)
{
 ...
 unsigned int i, j, k, l, i0 = 0, j0 = 0, k0 = 0, l0 = 0, t, boffset = 0;
 Xtest ptest;
 //char w[32][32][32][32];
 char**** w;
 w = (char****)calloc(32, sizeof(char***));

 for (i = 0; i < 32; i++)
 {
	 w[i] = (char***)calloc(32, sizeof(char**));
	 for (j = 0; j < 32; j++)
	 {
		 w[i][j] = (char**)calloc(32, sizeof(char*));

		 for (k = 0; k < 32; k++)
		 {
			 w[i][j][k] = (char*)calloc(32, sizeof(char));
			 memset(w[i][j][k], 0, sizeof(char)*32);
		 }
	 }
 }
